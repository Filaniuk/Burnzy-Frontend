...
import type { NicheMetricNotes, NicheMetrics } from "@/types/keywords";
...
function MetricRow({
  ...
  return (
    <div
      className={[
        // Less “curved/pill” look: subtle radius, crisp borders, flatter background
        "flex items-center justify-between gap-4",
        "rounded-lg border border-white/10",
        "bg-white/[0.03]",
        "px-4 py-3",
        emphasis ? "bg-white/[0.05]" : "",
      ].join(" ")}
    >
...
export default function NicheMetricsCard({ metrics, metricNotes }: Props) {
  ...
  const hasNotes = Boolean(notes.demand || notes.momentum || notes.opportunity || notes.competition);
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-[#151326] to-[#0F0E17] p-6 shadow-lg"
    >
      <div className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-[#6C63FF] via-[#00F5A0] to-[#6C63FF] opacity-80" />

      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl font-bold text-white">Probe Metrics</h3>
      ...
      {/* Flatter, less “curvy” rows; more like the screenshot layout */}
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <MetricRow label="Period" value={`${m.window_days ?? "-"} days`} />
        ...
        <MetricRow label="Total Sample Views (21d)" value={formatNumberCompact(view21d)} emphasis />
        <MetricRow label="Total Sample Views (7d)" value={formatNumberCompact(view7d)} emphasis />

        <MetricRow
          label="Median views/day (21d)"
          value={formatNumberCompact(m.views_per_day_21d_sample || 0)}
        />
        <MetricRow
          label="Median views/day (7d)"
          value={formatNumberCompact(m.views_per_day_7d_sample || 0)}
        />
      </div>
